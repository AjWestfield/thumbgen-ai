...
export type AetherHeroProps = {
  ...
  showDefaultContent?: boolean; // Whether to show default title/subtitle/cta
};

/* Glowing grid shader - bright glowing grid */
...
void main() {
  ...
  // Glowing grid lines
  float lineGlow = 1.0 - smoothstep(0.0, 1.5, gridLine);
  col += lineGlow * 0.15 * vec3(1.0, 1.0, 1.0);

  // Bright glow at intersections
  vec2 gridPos = fract(uv / gridSpacing);
  float distToIntersection = length(gridPos - 0.5) * 2.0;
  float intersectionGlow = exp(-distToIntersection * 4.0);

  // Pulsing effect for intersections
  float pulse = 0.7 + 0.3 * sin(T * 1.5);
  col += intersectionGlow * pulse * 0.4 * vec3(1.0, 0.95, 0.9);

  // Add outer glow bloom
  float bloom = exp(-distToIntersection * 2.0) * 0.15;
  col += bloom * vec3(0.8, 0.7, 0.6);

  // Subtle animated shimmer on grid lines
  float shimmer = 0.5 + 0.5 * sin(T * 2.0 + uv.x * 10.0 + uv.y * 10.0);
  col += lineGlow * shimmer * 0.05;
